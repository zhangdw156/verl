...
- **文件**：`verl/trainer/ppo/ray_trainer.py`，方法 `init_workers`（约 768–846 行）。
...
- **约 1461–1501 行**：  
  ...
  - **compute_advantage(batch, adv_estimator=config.algorithm.adv_estimator, ..., config=config.algorithm)**（`ray_trainer.py` 约 186 行）：  
    - 根据 `adv_estimator` 调用 **GAE / GRPO / EGPO** 等（在 `core_algos.py` 里实现），用 `token_level_rewards`、`response_mask`、`uid`（及 EGPO 时的 `entropys` 和 `responses`）算出 **advantages** 和 **returns**，写回 `batch.batch["advantages"]`、`batch.batch["returns"]`；  
...
| advantage      | GAE/GRPO/EGPO 用 rewards、uid、可选 entropys 和 responses | `compute_advantage`、`core_algos.py` |
